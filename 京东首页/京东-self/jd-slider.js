...
$('.ulWrapper',this.wrap)
    .css({listStyle:'none',
            width:this.type == 'animate' ? $('.liWrapper',this.wrap).size() * this.width : this.width,
            height:this.height,
            position:'absolute',
            left:0})
...
oslider.prototype.move = function(dir){
    ...
    switch(dir){
        case 'prev':
            if(this.type == 'animate'){
                if(self.pic == 0){
                    $('.ulWrapper',this.wrap).css({left:-($('.liWrapper',self.wrap).size() - 1)*self.width});
                    self.pic = $('.liWrapper',this.wrap).size() - 2;
                ...
                if(self.pic == 0){
                    self.pic = $('.liWrapper',this.wrap).size() - 1;
        ...
        case 'next':
            if(this.type == 'animate'){
                if(self.pic == $('.liWrapper',this.wrap).size() - 1){
  ...
  $('.spot .innerSpot',this.wrap).css({backgroundColor:'#fff'}).eq(this.type == 'animate' ? self.pic % ($('.liWrapper',this.wrap).size() - 1) : self.pic % ($('.liWrapper',this.wrap).size())).css({
    backgroundColor:'red'
  })
  }
